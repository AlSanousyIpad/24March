...
class PatientApp:
    def __init__(self, root):
        ...
        self.export_all_button = ttk.Button(root, text="Export All", command=self.export_all_patients)
        self.export_all_button.grid(row=0, column=5, padx=10, pady=10)
...
class PatientApp:
    ...
    def export_all_patients(self):
        ...
        if not patients:
            messagebox.showwarning("No Data", "No patients found to export.")
            return
        ...
        for patient in patients:
            ...
            pdf.cell(200, 10, txt="Patient Details", ln=True, align="C")
            pdf.cell(200, 10, txt="----------------", ln=True, align="C")
            pdf.cell(200, 10, txt=f"Name: {patient[1]}", ln=True)
            pdf.cell(200, 10, txt=f"Date: {patient[2]}", ln=True)
            pdf.cell(200, 10, txt=f"Complaint: {patient[3]}", ln=True)
            pdf.cell(200, 10, txt=f"Diagnosis: {patient[4]}", ln=True)
            pdf.cell(200, 10, txt=f"Treatment: {patient[5]}", ln=True)
            pdf.cell(200, 10, txt=f"Next: {patient[6]}", ln=True)
            pdf.ln(10)  # Add some space between patients

        # Save the PDF
        pdf_file = "all_patients.pdf"
        pdf.output(pdf_file)

        # Notify the user
        messagebox.showinfo("Export Successful", f"All patient details exported to {pdf_file}")
